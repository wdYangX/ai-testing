...
def validation_model_api(zip_file):
    ...
    data = {"model_name": "yolov8m_g", "confidence_threshold": "0.5", "iou_threshold": "0.5", }
...
def extract_and_convert_labels(zip_file_path):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        label_files = [f for f in os.listdir(os.path.join(temp_dir, file_list[0])) if f.endswith('.txt')]
        if not label_files:
        ...
        for label_file in label_files:
            file_path = os.path.join(temp_dir, file_list[0], label_file)
            try:
...
def model_metrics(data_file, label_file):
    ...
    for predict in predictions:
        ...
        metric = calculate_metrics(label, predict_lb)

...
bounding_boxes = df['boxes']  # Bounding boxes (predictions)
growth_boxes = df['g_boxes']  # Growth boxes (ground truth)
predictions = df['predict']  # Predicted class IDs
ground_truths = df['ground_truth']  # Ground truth class IDs
scores = df['scores']  # Prediction scores
...
def display_image_with_boxes(index, selected_classes, show_predictions, show_ground_truth, show_text_predictions,
    ...
    box_data = ast.literal_eval(bounding_boxes[index])  # Predictions
    growth_data = ast.literal_eval(growth_boxes[index])  # Ground truth
    pred_data = ast.literal_eval(predictions[index])  # Predicted class IDs
    gt_data = ast.literal_eval(ground_truths[index])  # Ground truth class IDs
...
def save_validation_status(st, status):
    if status == 'Approved':
        ...
        status = False
    df.at[st.session_state.img_index, 'confirm'] = status
...
def views(st):
    ...
    image_name = image_names[st.session_state.img_index]
    ...
    with col2:
        ...
        with c2:
            ...
            st.write(f"Image {st.session_state.img_index + 1}/{total_images}")
        with c3:
            if st.button("Next ➡️"):
    ...
    with col1:
        st.image(
            display_image_with_boxes(st.session_state.img_index, selected_classes, show_predictions, show_ground_truth,
                                     show_text_predictions, show_text_ground_truth),
            caption=f"Image {st.session_state.img_index + 1}/{total_images}", use_container_width=True, )
        st.markdown(f"### Image Name: {image_name}")
        c111, c222, c333, c444 = st.columns([1, 1, 1, 1])
        mapped_predictions = map_classes_with_colors(eval(f'{predictions[st.session_state.img_index]}'))
        mapped_ground_truths = map_classes_with_colors(eval(f'{ground_truths[st.session_state.img_index]}'))
        ...
        st.markdown(f"**Ground Truth (Bounding Boxes):** {create_html_with_color(mapped_ground_truths)}",
                    unsafe_allow_html=True)
        with c111:
            st.markdown(f"**Accuracy:** {accuracy[st.session_state.img_index]}")
        with c222:
            st.markdown(f"**Precision:** {precision[st.session_state.img_index]}")
        with c333:
            st.markdown(f"**Recall:** {recall[st.session_state.img_index]}")
        with c444:
            st.markdown(f"**F1-Score:** {f1_score[st.session_state.img_index]}")
